...
global.GetUnbusySpawn = function(room,pos)
{
    ...
    {
        if(Memory.sources[sources[i].id].busy < 30)
        {
...
global.GetUnbusySource = function(room,pos,neighboors)
{
...
global.FreeDeadOrders = function()
{
    for(var i in Memory.orders)
    {
...
global.ClearDeadCreeps = function()
{
...
//Правильно освобождаем память мертвых крипов
global.FreeDeadCreeps = function()
{
    //осовобождаем приказы приписанные к мертвым
    FreeDeadOrders();
    //уничтожаем мертвых крипов
